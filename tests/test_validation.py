...
from schematics.exceptions import ConversionError, ValidationError, StopValidationError, DataError
...
def test_type_validator_inheritance():
    ...
    assert (
        set(validators_custom[2:4])
        == set(validators_special[2:4])
        == set(["validate_", "validate_foo"])
    )  # both from CustomIntType; mutual order is random
